...
- [x] Dust emission (Draine & Li 2007 grids, or THEMIS with `CSPBasis(add_dust_emission=True, duste_model="THEMIS")`)
...
| Tests, FSPS-free | `pytest -m "not fsps and not gpu" -q -ra` | units, conventions, broadening, likelihood, samplers, post-processing, the misuse guards | every push and pull request |
| Regression baselines | `pytest tests/regression/test_regression.py -q` | 18 categories (SSP spectrum, IGM, cosmology, CSP components, dust attenuation, dust emission, nebular, CSP spectrum, and the likelihood, outlier, GP, calibration, marginalised-polynomial, emission-line-marginal, damped-IGM, dust-law, THEMIS and LSF-scale blocks; plus 4 optional ones skipped where their grid is absent) against stored arrays at `atol=1e-10, rtol=1e-7`, and a maximum relative residual of 1e-6; writes comparison figures to `tests/regression/figures/` | no, needs `$SPS_HOME` |

...
def run_scenarios():
    ...
    scenarios = [
        ...
        ("SedModel(zred < 0) without lumdist_mpc", {"ERROR"},
         lambda: SedModel(csp, [Photometry(filters=["sdss_g0", "sdss_r0"], flux=[1.0, 1.0],
                                           uncertainty=[0.1, 0.1], name="p")], zred=-0.001)),
        ...
        ("surviving-mass table above the formed mass (4.62 per M_sun)", {"ERROR"},
         lambda: _ssp.with_stellar_mass(np.full(np.shape(_ssp.ssp_flux)[:-1], 4.62),
                                        source="misuse")),
        ...
        ("GP + marginalised calibration polynomial (names route)", {"ERROR"},
         lambda: _gp_setup(_GP, polynomial_order=2, polynomial_mode="marginalize",
